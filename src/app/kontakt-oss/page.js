...
const KONTAKT_URI = "/vi-vil-hore-fra-deg/";
...
export default async function KontaktOssPage() {
  ...
  try {
    cmsContent = await getWpNodeByUri(KONTAKT_URI, revalidate);
  } catch {
    cmsContent = null;
  }
    ...
    <div className="min-h-screen bg-[#1f2730] text-white">
      ...
      <div className="relative">
        ...
        <main className="relative mx-auto w-full max-w-4xl px-6 py-20">
          ...
          <h1 className="mb-6 text-4xl font-extrabold">{cmsContent?.title ?? "La oss h\u00f8re fra deg"}</h1>
          ...
          {cmsContent ? (
            ...
            <p className="border-l-[3px] border-[#415791] pl-6 mb-6 text-xl leading-8 text-white/85">
              Fant ikke innhold fra WordPress akkurat n\u00e5. Sjekk at innlegget p\u00e5 <code>/vi-vil-hore-fra-deg/</code> finnes og at GraphQL-endpointet er tilgjengelig.
            </p>
          )}
        </main>
      </div>
